...
public class YTAppWidgetProvider_4x4 extends AppWidgetProvider {

	@Override
	public void onUpdate(Context context, AppWidgetManager appWidgetManager, int[] appWidgetIds){

		MyLog.d("YTAppWidgetProvider_4x4", "onUpdate");
...
//					+ " Maintable Lessons exist!");
	...
	public RemoteViews getWidgetViews(Context context, Timetable mainTable, int appWidgetId){
		...
		int w = ((int) context.getResources().getDimension(R.dimen.appwidget_4x4_minWidth));
		int h = ((int) context.getResources().getDimension(R.dimen.appwidget_4x4_realHeight));
	...
	public void onReceive(Context context, Intent intent) {
		...
		MyLog.d("YTAppWidgetProvider_4x4", "onReceive");
		if (intent.getAction().equals(Actions.YT_ACTION_TIMETABLE_DATA_CHANGED_4x4)) {
			// handle intent here
			AppWidgetManager appWidgetManager = AppWidgetManager.getInstance(context);
			ComponentName thisAppWidget = new ComponentName(
					context.getPackageName(), YTAppWidgetProvider_4x4.class.getName());
			int[] appWidgetIds = appWidgetManager.getAppWidgetIds(thisAppWidget);

			onUpdate(context, appWidgetManager, appWidgetIds);

			//String s = intent.getStringExtra("NewString");
			//			MyLog.d("YTAppWidgetProvider_4x4", "YT DATA CHANGED!");
		}
	}

	public static void onTimetableDataChanged(Context context){
		MyLog.d("YTAppWidgetProvider_4x4", "onTimetableDataChanged");
		Intent intent = new Intent(Actions.YT_ACTION_TIMETABLE_DATA_CHANGED_4x4);
